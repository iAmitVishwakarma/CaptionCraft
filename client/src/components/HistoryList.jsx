  ...
  return (
    <div className="space-y-6 overflow-y-auto lg:h-[calc(100vh-14rem)] h-[calc(100vh-6rem)] pr-2 custom-scrollbar">
      {history.map((post) => (
        <div
          key={post._id}
          className="bg-white/50 backdrop-blur-sm border border-gray-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-300 group"
        >
          <div className="flex gap-4 items-start">
            {/* Thumbnail */}
            <div className="w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
              <img
                src={`${post.image}?tr=w-150,h-150,fo-auto`}
                alt="Uploaded content thumbnail"
                width="80"
                height="80"
                className="w-full h-full object-cover"
                loading="lazy"
                fetchPriority={history.indexOf(post) === 0 ? "high" : "auto"}
              />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="space-y-2">
                {post.captions &&
                  Object.values(post.captions).map((cap, idx) => (
                    <div
                      key={idx}
                      className="bg-white rounded-md p-2 text-sm text-gray-700 border border-transparent hover:border-blue-200 transition-colors relative group/caption"
                    >
